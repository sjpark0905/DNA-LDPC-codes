...
for iter_num2 in range(start,end):
    ...
    for i in fail_DNA2:
        dec_file = "dec_codeword_n18432_m1860_%d" % i
        dec_output2 = file_read(dec_file,int)
        ...
        for j in range(18432):
            ...
            if dec_input[j]==0:
                num_min_llr = num_min_llr+1
                if dec_output2[j] == 1:
                    dec_input[j] = -0.1*math.log((1-epsil)/epsil)
                else:
                    dec_input[j] = 0.1*math.log((1-epsil)/epsil)
                
            
            # if dec_input[j]>=3*math.log((1-epsil)/epsil):
            #     dec_input[j] = 40
            # elif dec_input[j]<=-3*math.log((1-epsil)/epsil):
            #     dec_input[j] = -40
    ...
    # ===============================================================================================
    # Third decoding start
    # ===============================================================================================
    
    # print(fail_DNA)
    
    if fail_DNA2:
        print("('************** Start LDPC re-re-decoding **************\n")
        print('Number of erasure strand: %d\n' % len(erasure_index))
        
    fail_DNA3 = []
    re_decode_num = 0
    time_third_dec = time.time()
    ...
    for i in fail_DNA2:
        ...
        dec_input = file_read(soft_input,float)
        dec_output3 = file_read(dec_file,int)
        codeword = file_read(codeword_file,int)
        v = 0
        ...
        if v !=0:
            ...
            fail_DNA3.append(i)
            # print('index:%d\terrors:%d' %(i,v),"\t%d/%d" %(re_decode_num,len(fail_DNA)))
    print('Third decoding finished')
    print("Third decoding time:", time.time()-time_third_dec,"sec\n\n")
    
    # iter_num = 1
    f = open('%d_%d_%d_%f_result.txt' %(RS,th,iter_num2,epsil), 'w')
    f.write('==============================================================================\n')
    f.write('                               Results                                        \n')
    f.write('==============================================================================\n')
    ...
    if not fail_DNA2:
        ...
        f.write(f22)
        f3 = "Third decoding result:  %d/272\n" %(272-len(fail_DNA3))
        f.write(f3)
        ...
        if not fail_DNA2:
            ...
            for val in fail_DNA2:
                f.write(str(val)+' ')
            f.write('\n')
        f.write("Third decoding failure index: ")
        if not fail_DNA3:
            f.write("None\n")
        else:
            for val in fail_DNA3:
                f.write(str(val)+' ')
            f.write('\n')
    else:
        ...
        f.write(f5)
        f6 = "Third decoding result:\t%d/272\n" %(272-len(fail_DNA3))
        f.write(f6)
        f.write("First decoding failure index: ")
        if not fail_DNA:
            f.write("None\n")
    ...
    if not fail_DNA2:
        ...
        print("Third decoding result:  %d/272\n" %(272-len(fail_DNA3)))
