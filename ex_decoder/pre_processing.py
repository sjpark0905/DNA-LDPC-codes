...
index = np.zeros((pow(2,index_K-2),index_K-2),int)                                 
...
for i in range(pow(2,index_K-2)):
    if DNA_index[i][2] == DNA_index[i][3] or DNA_index[i][5] == DNA_index[i][6]:      
        erase.append(i)
    else:
...
binary_index_final = np.zeros((2*DNA_index_final.shape[0],index_K),int)          
