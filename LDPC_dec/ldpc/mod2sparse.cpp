...
{ 
	...
	//{ 
	//	
	//	while( ((m->rows[i]).right)->row != -1 )
	//	{
	//	e = (m->rows[i]).right;
	//	(m->rows[i]).right = e->right;
	//	free(e);
	//	}
	//}
...
)
{
	...
	if (mod2sparse_rows(m)>mod2sparse_rows(r) || mod2sparse_cols(m)>mod2sparse_cols(r))
	{ 
...
{ 
	...
	{ 
		fprintf(stderr,"mod2sparse_copyrows: Destination matrix has fewer columns than source\n");
		exit(1);
	}
...
{ 
	...
	{ 
		fprintf(stderr,"mod2sparse_copycols: Destination matrix has fewer rows than source\n");
		exit(1);
	}
...
)
{
	...
	if (mod2sparse_rows(m1)!=mod2sparse_rows(m2) || mod2sparse_cols(m1)!=mod2sparse_cols(m2))
	{ 
...
)
{
	...
	if (mod2sparse_rows(m)!=mod2sparse_cols(r) || mod2sparse_cols(m)!=mod2sparse_rows(r))
	{ 
	...
	if (r==m)
	{ 
		fprintf(stderr, "mod2sparse_transpose: Result matrix is the same as the operand\n");
		exit(1);
	}
...
)
{
	...
	if (mod2sparse_rows(m1)!=mod2sparse_rows(r) || mod2sparse_cols(m1)!=mod2sparse_cols(r) 
		|| mod2sparse_rows(m2)!=mod2sparse_rows(r)|| mod2sparse_cols(m2)!=mod2sparse_cols(r)) 
	{ 
	...
	if (r==m1 || r==m2)
	{ 
		fprintf(stderr,"mod2sparse_add: Result matrix is the same as one of the operands\n");
		exit(1);
	}
...
)
{
	...
	if (mod2sparse_cols(m1)!=mod2sparse_rows(m2) || mod2sparse_rows(m1)!=mod2sparse_rows(r) 
		|| mod2sparse_cols(m2)!=mod2sparse_cols(r))
	{ 
		fprintf (stderr,"mod2sparse_multiply: Matrices have incompatible dimensions\n");
		exit(1);
	}

	if (r==m1 || r==m2)
	{ 
		fprintf(stderr,"mod2sparse_multiply: Result matrix is the same as one of the operands\n");
		exit(1);
	}
...
)
{
	...
	{ 
		if (u[j])
		{
			for (e = mod2sparse_first_in_col(m,j);!mod2sparse_at_end(e);e = mod2sparse_next_in_col(e))
			{ 
	...
	{ 
		if (u[j])
		{
			...
			{ 
				...
				if(mod2sparse_row(e) < C_End && mod2sparse_row(e)>=C_Start)
				v[mod2sparse_row(e)] ^= 1;
			}
		}
	}
...
)
{
	...
	for (e = mod2sparse_first_in_row(m,row);!mod2sparse_at_end(e);e = mod2sparse_next_in_row(e))
	{ 
		count += 1;
	}
...
)
{
	...
	for (e = mod2sparse_first_in_col(m,col);!mod2sparse_at_end(e);e = mod2sparse_next_in_col(e))
	{ 
		count += 1;
	}
...
)
{
	...
	{ 
		fprintf (stderr,"mod2sparse_add_row: row added to is shorter than row added from\n");
		exit(1);
	}

	if (row1<0 || row1>=mod2sparse_rows(m1) || row2<0 || row2>=mod2sparse_rows(m2))
	{ 
...
)
{
	...
	{ 
		fprintf (stderr,"mod2sparse_add_col: Column added to is shorter than column added from\n");
		exit(1);
	}

	if (col1<0 || col1>=mod2sparse_cols(m1) || col2<0 || col2>=mod2sparse_cols(m2))
	{ 
...
{  
	...
	if (mod2sparse_cols(L)!=K || mod2sparse_rows(L)!=M || mod2sparse_cols(U)!=N || mod2sparse_rows(U)!=K)
	{ 
...
)
{
	...
	{ 
		...
		{ 
			fprintf(stderr,"mod2sparse_forward_sub: Matrix is not lower-triangular\n");
			exit(1);
		}
	}
...
)
{
	...
	{ 
		...
		{ 
			fprintf(stderr,"mod2sparse_backward_sub: Matrix is not upper-triangular\n");
			exit(1);
		}
	}
	...
	{ 
		...
		for (e = mod2sparse_first_in_row(U,i); !mod2sparse_at_end(e);e = mod2sparse_next_in_row(e))
		{ 
			j = mod2sparse_col(e);
