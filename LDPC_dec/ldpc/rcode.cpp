...
mod2sparse *forward_H;
mod2sparse *reverse_H;
...
mod2dense *G;		/* Dense or mixed representation of generator matrix,if type=='d' or type=='m' */
...
void make_check_H(mod2sparse *H,int decoder_type, int gap_PD,int N_PD, int M_PD)
{

	if(decoder_type ==0)
	{
		mod2sparse * temp = mod2sparse_allocate (gap_PD+M_PD,N);
		mod2sparse * temp1 = mod2sparse_allocate (gap_PD+M_PD,N);
		forward_H = mod2sparse_allocate(gap_PD+M_PD,gap_PD+N_PD);
		reverse_H = mod2sparse_allocate(gap_PD+M_PD,gap_PD+N_PD);

		for(int i=0; i<(gap_PD+M_PD); i++)
		{
			mod2sparse_add_row(temp,i,H,i);
		}
		for(int i=0; i<gap_PD+N_PD; i++)
		{
			mod2sparse_add_col(forward_H,i,temp,i);
		}

		for(int i=0; i<(gap_PD+M_PD); i++)
		{
			mod2sparse_add_row(temp1,i,H,i+gap_PD);
		}
		for(int i=0; i<gap_PD+N_PD; i++)
		{
			mod2sparse_add_col(reverse_H,i,temp1,i+gap_PD);
		}
		mod2sparse_clear(temp);
		mod2sparse_clear(temp1);
		mod2sparse_free(temp);
		mod2sparse_free(temp1);
	}
	else
	{
		mod2sparse * temp = mod2sparse_allocate (M_PD,N);

		forward_H = mod2sparse_allocate(M_PD,N_PD);

		for(int i=0; i<M_PD; i++)
		{
			mod2sparse_add_row(temp,i,H,i+gap_PD);
		}
		for(int i=0; i<N_PD; i++)
		{
			mod2sparse_add_col(forward_H,i,temp,i+gap_PD);
		}
...
//		if (M2!=M || N2!=N)
//		{ 
//			fprintf(stderr,"Generator matrix and parity-check matrix are incompatible\n");
//			exit(1);
//		}
//	}
//
