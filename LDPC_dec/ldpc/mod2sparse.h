#ifndef __MOD_2_SPARSE_H__
#define __MOD_2_SPARSE_H__
							 ...
							 the header for a row or column               */
{
	...
	double		msg_v_to_c_fp;
	double		msg_c_to_v_fp;

	//================================================================================
	// 전보환 수정
	// 기존의 float point를 사용하는 BP와는 다르게 유한한 수의 bit을 message로 사용하는
	// n-bit message passing decoder를 만들기 위해서 추가하였다.
	// mod2entry 객체는 parity check matrix H에서 nonzero element 즉, 1인 값을 뜻한다.
	// Tanner graph에서는 variable node에서 check node를 연결하는 edge를 의미하고,
	// 이 edge를 통해서 서로 message를 주고 받는다.  
	// 따라서 msg_v_to_c는 variable node(v)에서 check node(c)로 edge e=(v,c)를 통해 
	// 전달되는 v입장에서는 outgoing message를 뜻하고 c입장에서는 incoming message를 
	// 의미한다.
	// 비슷하게 msg_c_to_v는 c에서 v로 edge e=(c,v)를 통해 전달되는 msg를 의미한다.
	//
	// 2011.11.15
	//================================================================================
} mod2entry;
...
{ 
	int n_rows;		  /* Number o,f rows in the matrix */
...
int mod2sparse_decomp (mod2sparse *, int, mod2sparse *, mod2sparse *, int *, int *, mod2sparse_strategy, int, int);

int mod2sparse_forward_sub  (mod2sparse *, int *, char *, char *);
int mod2sparse_backward_sub (mod2sparse *, int *, char *, char *);

void mod2sparse_mulvec_bound ( 	mod2sparse *m,	char *u, char *v, int D_Start,int D_End,int C_Start,int C_End); //2018.08.29 곽희열 추가, Window decoder에서 partial syndrome 계산
