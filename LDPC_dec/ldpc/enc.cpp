...
)
{
	...
	{ 
		...
		if (sblk[j-M]==1)
		{ 
			for (e = mod2sparse_first_in_col(H,cols[j]);!mod2sparse_at_end(e);e = mod2sparse_next_in_col(e))
			{ 
	...
	if (!mod2sparse_forward_sub(L,rows,x,y)|| !mod2sparse_backward_sub(U,cols,y,cblk))
	{ 
		abort(); /* Shouldn't occur, even if the parity check matrix has redundant rows */
	}
...
)
{
	...
	{ 
		...
		{ 
			for (e = mod2sparse_first_in_col(H,cols[j]);!mod2sparse_at_end(e);e = mod2sparse_next_in_col(e))
			{ 
				mod2dense_flip(u,mod2sparse_row(e),0);
			}
		}
	}
